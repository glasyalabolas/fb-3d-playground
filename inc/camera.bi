...
constructor Camera( _
  ...
  /'
    ...
    Every time one messes with the Camera vectors directly, remember 
    to preserve their length, if you do not do this, the projection
...
function Camera.orthogonal( pnt as Vec4, plane as Rectangle, byref x as single, byref y as single, byref z as single ) as integer
  ...
  if( px >= 0 andAlso px < plane.width andAlso py >= 0 andAlso py < plane.height ) then
    x = px
    y = py
    z = b.z
...
function Camera.perspective( pnt as Vec4, plane as Rectangle, byref x as single, byref y as single, byref z as single ) as integer  
  ...
  if( px >= 0 andAlso px < plane.width andAlso py >= 0 andAlso py < plane.height andAlso b.z >= m_nearClip ) then
    x = px
    y = py
    z = b.z
