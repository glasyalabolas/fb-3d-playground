#ifndef __FB3DPG_OBJECT3D__
#define __FB3DPG_OBJECT3D__
...
type Object3D
  public:
    ...
    declare sub render( cam as Camera, context as any ptr = 0 )
  ...
  private:
    ...
    as Mat4 m_objMatrix, m_invObjMatrix
...
destructor Object3D()
  ...
  for i as integer = 0 to m_vertices.count - 1
    var v = m_vertices.get( i )
    delete( cast( Vec4 ptr, v ) )
  next
  
  '' Same for the edges
  for i as integer = 0 to m_edges.count - 1
    var e = m_edges.get( i )
    delete( cast( Edge ptr, e ) )
  next
end destructor
...
sub Object3D.generateMatrix()
  '' Generates the object's transform matrix
  m_objMatrix = Mat4( _
  ...
  '' Compute the inverse immediately
...
sub Object3D.setPitch( angle as single )
  rotate( m_u, angle - getPitch() )
end sub

sub Object3D.setRoll( angle as single )
  rotate( m_dir, angle - getRoll() )
end sub
...
sub Object3D.setDistance( pnt as Vec4, dist as single )
  move( distance( m_pos, pnt ) * normalize( pnt - pos ) )
end sub
...
sub Object3D.render( cam as Camera, context as any ptr = 0 )
  dim as Edge ptr e
  ...
  for j as integer = 0 to m_edges.count - 1
    ...
    '' Transform the points of the object to world space, as the drawLine3D() function expects them
    '' in world coordinates.				
    line3D( cam, objectSpaceToWorldSpace( *e->vertex1 ), objectSpaceToWorldSpace( *e->vertex2 ), color, context )			
  next
end sub
