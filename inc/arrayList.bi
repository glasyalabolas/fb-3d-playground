#ifndef __FB_ARRAYLIST__
#define __FB_ARRAYLIST__
...
/'
  ...
  Note that this class isn't meant to do garbage collection,
...
type ArrayList
  public:
    ...
    declare function get( index as ulongint ) as any ptr
    declare function count() as ulongint
    declare sub add( e as any ptr )
    declare sub insert( e as any ptr, before as ulongint )
    declare function remove( index as ulongint ) as any ptr
  ...
  #macro resizeElements( v )
    m_count += v : redim preserve m_element( 0 to m_count - 1 )
  #endmacro
end type

constructor ArrayList() : end constructor

function ArrayList.count() as ulongint
  return( m_count )
end function

function ArrayList.get( index as ulongint ) as any ptr
...
sub ArrayList.insert( e as any ptr, before as ulongint )
  ...
  if( before = m_count - 1 ) then
    ...
    dim as ulongint elem = m_count - 1 - before
...
function ArrayList.remove( item as any ptr ) as any ptr
  ...
  dim as longint elem = -1 '' assume not found
  
  '' Search it
  for i as ulongint = 0 to m_count - 1
...
function ArrayList.remove( index as ulongint ) as any ptr
  ...
  dim as ulongint elem = m_count - 1 - index
...
function ArrayList.removeFirst() as any ptr
  ...
  '' General case
  '' Calculate number of elements to move
  dim as ulongint elem = m_count - 1
