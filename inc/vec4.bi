#ifndef __FB3DPG_VEC4__
#define __FB3DPG_VEC4__	

#include once "math.bi"

/'
  Homogeneous 4 tuple type.
  
  It is meant to be used as an homogeneous 3D vector (like the ones used by OpenGL
  or Direct3D).
  ...
  03/31/2023: Refactor,
  10/07/2017: Fixed a typo in the + operator
    ...
    Goes to show you what happens when you are a fucking imbecile and write code in a hurry.
...
operator Vec4.cast() as string
  ...
  return( _
    "| " + trim( str( x ) ) + " |" + chr( 13, 10 ) + _
    "| " + trim( str( y ) ) + " |" + chr( 13, 10 ) + _
    "| " + trim( str( z ) ) + " |" + chr( 13, 10 ) + _
    "| " + trim( str( w ) ) + " |" + chr( 13, 10 ) )
end operator
...
function Vec4.cross( v as Vec4 ) as Vec4
  ...
  '/
  return( Vec4( v.y * z - v.z * y, v.z * x - v.x * z, v.x * y - v.y * x, 1.0 ) )
end function
...
operator / ( v as Vec4, a as single ) as Vec4
  dim as single ia = 1.0 / a
  return( Vec4( v.x * ia, v.y * ia, v.z * ia ) )
end operator
...
function vectorAngle( v as Vec4, w as Vec4 ) as single
  /'
    ...
    define scalar to be single instead of double). If that happens, the acos function
    will give an invalid result. So instead a protection was added that sets the value
    back to 1 or -1 (because, if the value became 1.0000023 for example, it was probably
    supposed to be 1 anyway).
  ...
  return( -acos( clamp( -1.0, 1.0, angleCos ) ) )
end function
